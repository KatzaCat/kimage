...
std::vector<unsigned char>  k::ImageProcessor::getData() {
        std::vector<unsigned char> raw_data;

        for (size_t y_index = this->getHeight(); y_index > 0; y_index--) {
                for (size_t x_index = 0; x_index < this->getWidth(); x_index++) {
                        uint32_t pixel = this->pixel_data.at(((y_index - 1) * this->getWidth()) + x_index);
...
bool k::ImageProcessor::populateDataFromFile(std::ifstream &file) {
        file.seekg(0, std::ios::end);
        std::streamsize file_size = file.tellg();
        file.seekg(0, std::ios::beg);

        if (file_size <= 0) {return false;}
        this->byte_data.resize(file_size);

        for (size_t index = 0; index < this->byte_data.size(); index++) {
                char current_byte;
                file.get(current_byte);

                this->byte_data[index] = std::byte(current_byte);
        }

        return true;
}

std::byte k::ImageProcessor::getByte(size_t offset) {
        if (offset >= this->byte_data.size() || offset < 0)
        {return std::byte(0);}
        return this->byte_data.at(offset);
}

uint16_t k::ImageProcessor::getTwoBytesBigEndian(size_t offset)
{return tools::toInt16(this->getByte(offset), this->getByte(offset + 1));}

uint32_t k::ImageProcessor::getFourBytesBigEndian(size_t offset) {
...
uint16_t k::ImageProcessor::getTwoBytesLittleEndian(size_t offset) {
...
uint32_t k::ImageProcessor::getFourBytesLittleEndian(size_t offset) {
